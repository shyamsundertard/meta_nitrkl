    ...
    <div className='flex flex-col items-center ml-[1px] min-w-[250px] min-h-[700px] border-r-2 border-cyan-800 rounded-lg pt-5 '>
      ...
      {pageInfoDetails?.map((page)=>(
        <Link href={`http://localhost:3000/${webPageName}/Page/${page.title}`}
          className='flex justify-center bg-white cursor-pointer items-center w-full h-[30px] border border-l-4 mb-[1px] transform transition-transform duration-300 border-gray-400 hover:bg-gray-100 hover:scale-105 hover:ml-2 hover:border-l-4 rounded-r-lg'
          key={page.id}>
          {page.title}
          </Link>
      ))}
